...
void main() {
  group('TiffChunkPlan.forBudget', () {
    test(
      'a generous budget produces one chunk per tile row (no shrinking below tile height)',
      () {
        final metadata = _tiledMetadata(
          width: 1000,
          height: 2048,
          tileWidth: 512,
          tileLength: 512,
        );
        final plan = TiffChunkPlan.forBudget(
          metadata,
    ...
    test(
      'a tight budget shrinks chunks below tile height rather than exceeding it',
      () {
        final metadata = _tiledMetadata(
          width: 10000,
          height: 2048,
    ...
    test(
      'never shrinks below minChunkHeight even for an absurdly small budget',
      () {
        final metadata = _tiledMetadata(
          width: 100000,
          height: 16,
    ...
    test(
      'a page shorter than the natural chunk height produces exactly one chunk',
      () {
        final metadata = _tiledMetadata(
          width: 100,
          height: 50,
    ...
    test(
      'the last chunk is shorter when height is not a multiple of chunkHeight',
      () {
        final metadata = _tiledMetadata(
          width: 100,
          height: 1000,
    ...
    test(
      'a strip-organized page uses rowsPerStrip as the natural chunk height',
      () {
        final metadata = _stripMetadata(
          width: 100,
          height: 400,
          rowsPerStrip: 100,
        );
    ...
    test('rejects a non-positive budget or minChunkHeight', () {
      ...
      expect(
        () => TiffChunkPlan.forBudget(metadata, maxBytesPerChunk: 0),
        throwsArgumentError,
      );
      expect(
        () => TiffChunkPlan.forBudget(
          metadata,
  ...
  group('TiffChunkPlan.recommendedWorkerCount', () {
    ...
    test(
      'falls back to 1 rather than 0 when one chunk alone exceeds the budget',
      () {
        final count = TiffChunkPlan.recommendedWorkerCount(
