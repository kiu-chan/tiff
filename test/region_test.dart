...
void main() {
  group('Region decode', () {
    test('strip layout only reads strips that intersect the requested region', () {
      ...
      final recording = RecordingByteSource(MemoryByteSource(builder.build()));
      ...
      final raster = image.decodeRegion(const TiffRegion(x: 1, y: 1, width: 2, height: 2));
      expect(raster.width, 2);
      expect(raster.height, 2);
      expect(raster.samples, [6, 7, 10, 11]);

      expect(recording.readOffsets, containsAll([stripOffsets[1], stripOffsets[2]]));
      expect(recording.readOffsets, isNot(contains(stripOffsets[0])));
      expect(recording.readOffsets, isNot(contains(stripOffsets[3])));
    });

    test('tile layout only reads tiles that intersect the requested region', () {
      ...
      final raster = image.decodeRegion(const TiffRegion(x: 2, y: 2, width: 2, height: 2));
    ...
    test('rejects an out-of-bounds region with a clear error', () {
      ...
      expect(
        () => image.decodeRegion(const TiffRegion(x: 1, y: 1, width: 5, height: 5)),
