...
String _writeTiledFixture(Directory dir, {required int width, required int height, required int tileSize}) {
...
void main() {
  ...
  setUp(() => tempDir = Directory.systemTemp.createTempSync('tiff_parallel_decoder_test_'));
  ...
  test('decodeBanded reassembles the exact same pixels as a direct whole-page decode', () async {
    const width = 40, height = 25, tileSize = 8;
    final path = _writeTiledFixture(tempDir, width: width, height: height, tileSize: tileSize);
    ...
    await TiffParallelDecoder.decodeBanded(
      ...
      onBand: (band) {
        ...
        assembled.setRange(band.y * rowBytes, (band.y + band.height) * rowBytes, band.rgba);
      },
    );
  ...
  test('workerCount greater than the number of chunks still works (no empty-worker crash)', () async {
    const width = 20, height = 8, tileSize = 32; // one chunk covers the whole page
    final path = _writeTiledFixture(tempDir, width: width, height: height, tileSize: tileSize);
    final reference = referenceRgba(path);
  ...
  test('a tight maxBytesPerChunk (forcing multiple chunks per tile row) still reassembles correctly', () async {
  ...
  test('throws a TiffException for an out-of-range pageIndex', () async {
    final path = _writeTiledFixture(tempDir, width: 16, height: 16, tileSize: 8);
    expect(
  ...
  test('propagates a worker decode failure as a TiffException instead of hanging', () async {
