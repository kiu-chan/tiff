...
void main() {
  ...
  group('a JPEG scan split across strips', () {
    test(
      'decode() reassembles it and matches decoding the original JPEG whole',
      () {
    ...
    test(
      'decodeRegion() still returns the right pixels, not just decode()',
      () {
        final source = gradient(16, 8);
        final jpegBytes = Uint8List.fromList(
          img.encodeJpg(source, quality: 90),
        ...
        final splitAt =
            jpegBytes.length ~/ 3; // an uneven split, for good measure
    ...
    test(
      'a shared JPEGTables stream still merges in correctly for a split scan',
      () {
        ...
        expect(
          sofStart,
          greaterThan(0),
          reason: 'test JPEG must contain a baseline SOF0 marker',
        );
        final jpegTables = Uint8List.fromList([
          ...full.sublist(0, sofStart),
          0xFF,
          0xD9,
        ]);
        ...
        final builder = TiffFixtureBuilder()
          ...
          ..addBytesTag(
            TiffTagId.jpegTables,
            TiffTagType.tUndefined,
            jpegTables,
          )
          ..addStripOffsetsTag(TiffTagId.stripOffsets, TiffTagType.tLong, [
            splitAt,
  ...
  group(
    'independent, self-contained per-tile JPEGs (e.g. a real scanner file)',
    () {
      test(
        'one tile failing for an unrelated reason surfaces its own error, not a stitching artifact',
        () {
          ...
          final tileAJpeg = Uint8List.fromList(
            img.encodeJpg(tileA, quality: 90),
          );
          final tileBWrongSize = gradient(4, 4);
          final tileBJpeg = Uint8List.fromList(
            img.encodeJpg(tileBWrongSize, quality: 90),
          );
      ...
      test(
        'every tile still decodes independently when all are well-formed',
        () {
          final tileA = gradient(8, 8);
          final tileAJpeg = Uint8List.fromList(
            img.encodeJpg(tileA, quality: 90),
          ...
          final tileBJpeg = Uint8List.fromList(
            img.encodeJpg(tileB, quality: 90),
          );
          final pixelData = Uint8List.fromList([...tileAJpeg, ...tileBJpeg]);
          ...
          final referenceA = img
              .decodeJpg(tileAJpeg)!
              .getBytes(order: img.ChannelOrder.rgb);
          final referenceB = img
              .decodeJpg(tileBJpeg)!
              .getBytes(order: img.ChannelOrder.rgb);
  ...
  group('sparse strips/tiles with a byte count of 0', () {
    test(
      'a sparse tile is skipped, not misread as a split JPEG scan needing stitching',
      () {
