...
void main() {
  group('SystemMemoryInfo.probe', () {
    test(
      'never throws, and returns internally-consistent numbers when supported',
      () {
        final mem = SystemMemoryInfo.probe();
        if (mem == null)
          return; // Unsupported here (e.g. CI sandbox) — nothing to assert.
  ...
  group('TiffAutoDecodeBudget.recommend', () {
    test(
      'falls back to fallbackAggregateBytes when told to ignore the system reading',
      () {
    ...
    test(
      'never recommends zero workers for a huge whole-slide-image-shaped page',
      () {
    ...
    test(
      'uses every idle core for a huge page even when the budget is too small for one full-tile-row chunk per core',
      () {
    ...
    test('scales worker count up for a small page given a generous budget', () {
      ...
      final budget = TiffAutoDecodeBudget.recommend(
        ...
        systemMemoryFraction:
            0, // force the fallback path, deterministic regardless of host machine
      ...
      expect(
        budget.workerCount,
        Platform.numberOfProcessors > 1 ? greaterThan(1) : 1,
      );
    });
  });
}
